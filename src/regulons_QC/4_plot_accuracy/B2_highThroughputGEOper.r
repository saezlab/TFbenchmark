...
load('data/regulons_QC/B2_perturbations/aggregated_activities_formated.rdata')
...
balanced_accuracy = F
plot_title = 'B2'
outdir = 'results/regulons_QC/B2_perturbations/'



# Counts
TFs = unique(df$perturbed_TF[ df$TF == df$perturbed_TF ])
write.table(TFs, file = 'data/regulons_QC/benchmark2_TFs.txt', quote = F, row.names = F, col.names = F)
num_TFs = length(TFs)
num_TFs
...
plot_PR(sdf, balanced_accuracy = balanced_accuracy, line_colors = line_colors) 
