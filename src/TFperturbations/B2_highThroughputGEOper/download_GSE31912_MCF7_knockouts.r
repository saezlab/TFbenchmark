#' This code downloads the expression data 
#' corresponding to the experiment GSE31912 
#' in the B2 benchmarck datasets (Garcia-Alonso et al. Cancer Research 2018)
...
save(Esymbol_zscore, file = 'data/regulons_QC/B2_perturbations/GSE31912/expression_signatures/MCF7_zscores.rdata')
...
ggsave(filename = 'data/regulons_QC/B2_perturbations/GSE31912/expression_signatures/MCF7_zscores_ranks.png', width = 12, height = 5)
...
ggplot(df, aes(y=rank_position, x=gene, label=label, color = color)) + geom_point() + geom_text_repel() + 
  ...
  theme_light() + theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) + scale_color_identity()
ggsave(filename = 'data/TF_target_sources/B2_perturbations/GEO/GSE31912_MCF7_TFknockdown_zscores_ranks_control.png', width = 12, height = 5)
non_reliable_tfs = c(non_reliable_tfs, df$gene[ df$rank_position < - 0.75 ])
write.table(non_reliable_tfs, file = 'data/regulons_QC/B2_perturbations/GSE31912/expression_signatures/MCF7_nonreliableTFperturbation.txt', row.names = F, col.names = F, quote = F)
