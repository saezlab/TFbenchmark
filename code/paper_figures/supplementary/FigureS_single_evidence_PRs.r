...
pA = list.files(path = 'results/regulons_QC/PR-AUC_balanced/',  recursive = T, full.names = T, pattern = 'chip') %>% 
  grep(., pattern = 'rdata', value = T) %>% 
  grep(., pattern = 'PR_', value = T)
...
pB = list.files(path = 'results/regulons_QC/PR-AUC_balanced/',  recursive = T, full.names = T, pattern = 'TFBS_size') %>% 
  grep(., pattern = 'rdata', value = T) %>% 
  grep(., pattern = 'PR_', value = T)
...
pC = list.files(path = 'results/regulons_QC/PR-AUC_balanced/',  recursive = T, full.names = T, pattern = 'TFBS_2') %>% 
  grep(., pattern = 'rdata', value = T) %>% 
  grep(., pattern = 'PR_', value = T)
...
pD = list.files(path = 'results/regulons_QC/PR-AUC_balanced/',  recursive = T, full.names = T, pattern = 'inferred') %>% 
  grep(., pattern = 'rdata', value = T) %>% 
  grep(., pattern = 'PR_', value = T)
...
pE = list.files(path = 'results/regulons_QC/PR-AUC_balanced/',  recursive = T, full.names = T, pattern = 'curated') %>% 
  grep(., pattern = 'rdata', value = T) %>% 
  grep(., pattern = 'PR_', value = T)
...
P = plot_grid(pA[[2]], pA[[3]], pA[[1]], 
              ...
              labels = c('A', '', '', 'B', '', '', 'C', '', '', 'D', '', '', 'E', '', ''), 
              label_size = 25)
save_plot(P, filename = 'paper/figures/supplementary/FigureS_single_evidence_PR.png', dpi = 300, base_height = 15, base_width = 16)
