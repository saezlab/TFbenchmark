...
mdf = rbind(data.frame(get(load(file = 'paper/figures/Figure3/enrichment_b1.rdata')), benchmark = 'b1'), 
            data.frame(get(load(file = 'paper/figures/Figure3/enrichment_b2.rdata')), benchmark = 'b2'),
            data.frame(get(load(file = 'paper/figures/Figure3/enrichment_b3.rdata')), benchmark = 'b3'))
...
P
ggsave(filename = 'paper/figures/Figure3/Figure3_2ndHalf.png', dpi = 300, height = 10, width = 10)
...
P = ggplot(x[ grep('class', x$label, invert = F) , ], aes(y = ES, x = label, fill = pval ) ) + geom_bar(stat = 'Identity')  + 
  ...
  mytheme + theme(strip.text.y = element_text(angle=0, hjust = 0), 
                  ...
                  plot.subtitle = element_text(face = "italic", hjust = 0.5))
P
ggsave(filename = 'paper/figures/Figure3/Figure3_3rdHalf.png', dpi = 300, height = 15, width = 10)
