...
setwd(home)
source('code/utils.r')
source('code/regulons_QC/plots/lib_plots_regulonsQC.r')
library(viper)
...
df$rank_pvalue1tailed = 1 - df$rank_pvalue1tailed # here we are looking at greater percentiles
...
for(dataset  in unique(df$regulon_dataset) ){
  ...
  for ( feature in TFrole_features ) {
    ...
    mrs = NULL
    try(mrs <- msviper( scale(mySignature[ names(mySignature) %in%  unlist(sapply(unlist(feature_network, recursive = F), names))])[,1], 
                       feature_network, verbose = F, minsize = 4, pleiotropy = F, ges.filter = T , adaptive.size = F), silent = T)
    if( is.null(mrs) )
      next()
...
p3c = ggplot(x, aes(y = NES, x = label, fill = p.value ) ) + geom_bar(stat = 'Identity')  + 
  coord_flip() +
...
p3c
ggsave('paper/figures/Figure3c.png', width = 12, height = 5, dpi = 300)
save(p3b, file = 'paper/figures/Figure3c.rdata')
