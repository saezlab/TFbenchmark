...
network_files = grep("RANDOM_NETWORKS_coexpressionPriors_tissueSpecific", network_files, value = T)
...
for (myFile in rev(perturbation_files)){
  ...
  analysis_name = gsub('contrasts/', 'activities/random_tissuespecific_coexpressionPriors/viper_', myFile)
