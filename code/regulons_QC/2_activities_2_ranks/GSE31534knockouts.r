...
tissue = 'Skin'
f = list.files('data/regulons_QC/pertubations/GSE31534/activities', full.names = T) %>% grep(.,  pattern = tissue, value = T) %>% grep(.,  pattern = 'aracne', value = T)
message(f)
nes = get(load(f))
if( ! is.matrix(nes) )
  next
...
save(activity_nes, 
     ...
     file = 'data/regulons_QC/pertubations/GSE31534/activity_comparison_results.rdata')
