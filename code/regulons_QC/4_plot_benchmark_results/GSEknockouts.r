...
df = subset(df, ! regulon_dataset %in% c('all_weighted', 'cnio', 'inferredGTEx_dynamic', 'TOP') )
...
p = plot_boxplots(high_conf, wrap_variable = 'TF_type')
p[[2]]
p = plot_boxplots(high_conf, wrap_variable = 'expreiment_GEOid')
p[[2]]
...
x_scale = c(min(P$aucs$AUC), round(max(P$aucs$AUC, 1)))
