...
oreganno = unique(subset(data, V13 %in% c("N/A", 'STAT1 literature-derived sites'))[, c('TF', 'target', 'effect') ])
...
write.table(oreganno, file = 'data/TF_target_sources/curated_databases/oreganno/network.sif', sep = '\t', col.names = T, row.names = F, quote = F)
